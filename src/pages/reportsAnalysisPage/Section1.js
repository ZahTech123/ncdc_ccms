...
const Section1 = ({ tickets = [] }) => { // Provide a default value for tickets
  ...
  useEffect(() => {
    if (!donutChartRef.current) return; // Ensure the canvas element exists

    const ctxDonut = donutChartRef.current.getContext("2d");
    const resolvedCount = tickets.filter(ticket => ticket.status === "Resolved").length;
    const pendingCount = tickets.filter(ticket => ticket.status === "Pending").length;
    const complaintsData = [resolvedCount, pendingCount];
    const totalComplaints = complaintsData.reduce((a, b) => a + b, 0);

    const gradientResolved = ctxDonut.createLinearGradient(0, 0, 0, 400);
    gradientResolved.addColorStop(0, "#f8df7c");
    gradientResolved.addColorStop(1, "#eab308");

    const gradientPending = ctxDonut.createLinearGradient(0, 0, 0, 400);
    gradientPending.addColorStop(0, "#c59f09");
    gradientPending.addColorStop(1, "#eab308");
          ...
          <div className="flex space-x-6">
            <div className="flex-1">
              <table className="w-full table-auto text-sm">
                <tbody>
                  {[
                    { label: "Total Tickets", value: tickets.length, color: "gray-500" },
                    { label: "Total New", value: tickets.filter(ticket => ticket.status === "New").length, color: "green-500" },
                    { label: "Total In Progress", value: tickets.filter(ticket => ticket.status === "In Progress").length, color: "yellow-500" },
                    { label: "Total Resolved", value: tickets.filter(ticket => ticket.status === "Resolved").length, color: "blue-500" },
                  ].map((row, index) => (
            ...
            <div className="flex-1">
              <table className="w-full table-auto text-sm">
                <tbody>
                  {[
                    { label: "Total Overdue", value: tickets.filter(ticket => ticket.status === "Overdue").length, color: "red-500" },
                    { label: "Total High Priority", value: tickets.filter(ticket => ticket.priority === "High").length, color: "red-600" },
                    { label: "Total Medium Priority", value: tickets.filter(ticket => ticket.priority === "Medium").length, color: "yellow-400" },
                    { label: "Total Low Priority", value: tickets.filter(ticket => ticket.priority === "Low").length, color: "green-600" },
                  ].map((row, index) => (
                    <tr key={index} className="border-b border-gray-600">
                      <td className="p-2 text-left">{row.label}</td>
                      <td className="p-2 text-left">
